...
HRESULT CLevel_RoyalGarden::Ready_Layer_BackGround(const _tchar * pLayerTag)
{
	...
	if (FAILED(m_pGameInstance->Add_GameObject_To_Layer(LEVEL_ROYALGARDEN, TEXT("Prototype_GameObject_Terrain"), LEVEL_ROYALGARDEN, pLayerTag, &pDesc)))
		return E_FAIL;
...
HRESULT CLevel_RoyalGarden::Ready_Layer_Structure(const _tchar* pLayerTag)
{
	...
	//	//int b = 4;
	//	if (FAILED(m_pGameInstance->Add_GameObject_To_Layer(LEVEL_ROYALGARDEN, Translate_wchar, LEVEL_ROYALGARDEN, Layer_Name)))
	//		return E_FAIL;
	...
	//	auto& Level_RoyalGarden = Level_Layers[3];

	//	for (auto& Layers : Level_RoyalGarden)
	//	{
	//		//auto& iter = find(Level_RoyalGarden.begin(), Level_RoyalGarden.end(), Layer_Name);	
	//		auto iter = Level_RoyalGarden.find(Layer_Name);

	//		if (iter == Level_RoyalGarden.end())
	//			return E_FAIL;
...
HRESULT CLevel_RoyalGarden::Ready_Layer_Effect(const _tchar* pLayerTag)
{
	//Mesh Effect
...
HRESULT CLevel_RoyalGarden::Ready_Layer_Item(const _tchar* pLayerTag)
{
	...
	if (FAILED(m_pGameInstance->Add_GameObject_To_Layer(LEVEL_STATIC, TEXT("Prototype_GameObject_GameItem"), LEVEL_ROYALGARDEN, pLayerTag, &ItemDesc)))
		return E_FAIL;
...
HRESULT CLevel_RoyalGarden::Ready_Layer_UIGroup_PlayerLevelUP(const _tchar* pLayerTag)
{
	...
	if (FAILED(m_pGameInstance->Add_GameObject_To_Layer(LEVEL_STATIC, TEXT("Prototype_GameObject_UIGroup_PlayerLevelUP"), LEVEL_ROYALGARDEN, pLayerTag, &Desc)))
		return E_FAIL;
	return S_OK;
}
...
{
	if (FAILED(m_pGameInstance->Add_GameObject_To_Layer(LEVEL_STATIC, TEXT("Prototype_GameObject_UIGroup_Inventory"), LEVEL_ROYALGARDEN, pLayerTag, nullptr, "Inventory")))
		return E_FAIL;
	return S_OK;
}
...
HRESULT CLevel_RoyalGarden::Load_Objects(_int iObject_Level)
{
	...
	for (_uint i = 0; i < iSize2; ++i)
	{
		...
		CEnvironmentObject* pEnvironment = reinterpret_cast<CEnvironmentObject*>(m_pGameInstance->Add_GameObject_To_Layer_Take(LEVEL_ROYALGARDEN, TEXT("Prototype_GameObject_Object_GroundObject"), LEVEL_ROYALGARDEN, TEXT("Layer_GroundObject"), &Desc));
...
HRESULT CLevel_RoyalGarden::Load_Effect(const _tchar* _pEffectFilePath, _uint _iPrototypeLevelIndex, const _tchar* _pEffectPrototypeName, EFFECT_TYPE _eEffectType, EFFECT_NAME _eEffectName, _uint _iEffectCount)
{
